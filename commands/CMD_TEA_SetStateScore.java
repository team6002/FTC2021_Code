...
public class CMD_TEA_SetStateScore extends SequentialCommandGroup {
    private final FSM_TeamElementArm m_FSM;

    public CMD_TEA_SetStateScore(FSM_TeamElementArm p_FSM
        , SUB_TEA_elbow p_elbow, SUB_TEA_wrist p_wrist, SUB_TEA_claw p_claw) 
    {
        ...
        addCommands (
            ...
            (
                new CMD_TEA_ElbowSetScore(p_elbow)
                , new CMD_TEA_WristSetScore(p_wrist)
                , new CMD_TEA_ClawSetClose(p_claw)
            )
            // , new Sleep(200)
            // , new CMD_TEA_ClawSetOpen(p_claw)
            // , new CMD_TEA_ElbowSetReadyToScore(p_elbow)
            , new CMD_TEA_SetState(p_FSM, FSM_TeamElementArm.State.SCORE)
        );
    }
}
