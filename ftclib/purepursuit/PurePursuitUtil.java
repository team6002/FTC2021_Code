...
public final class PurePursuitUtil {
    ...
    public static double[] moveToPosition(double cx, double cy, double ca, double tx, double ty, double ta, boolean turnOnly) {
        ...
        if (turnOnly) {
            ...
            double powerTurn = angleWrap(ca - ta) / Math.toRadians(30);
            powerTurn = Range.clip(powerTurn, -1, 1);
            
            return new double[] {0, 0, powerTurn};
        }
        ...
        double relativeAngleToPosition = angleWrap(absoluteAngleToPosition - ( ca - Math.toRadians(90)));
        ...
        double relativeTurnAngle = angleWrap(ca - ta);
        ...
        double powerTurn = relativeTurnAngle / Math.PI;
        
        // using this causes CMD_DriveGotoPosition problem---overwhelming the forward/strafe speed values?
        // double powerTurn = relativeTurnAngle / Math.toRadians(30);
        // powerTurn = Range.clip(powerTurn, -1, 1);
